...
| Access decision | Application-managed self-registration and administrator approval are implemented locally; target-state email verification, participant-data rules, and staging evidence remain open (D-23 (How should participants obtain access to the Option B application?), D-24 (May local development bypass email verification while Mailpit delivery is unavailable?), DEP-09 (Option B authentication and participant-data handling decision)) | `pm-control/registers/decision-log.md`; `pm-control/registers/dependencies-log.md` |
| Build           | In progress; local participant self-registration, member access management, audit logging, and protected access controls are implemented and conditionally measured; staging verification and participant-data rules remain | `application/`; `pm-control/project-control.md`; `pm-control/registers/action-log.md` |
