import type { InertiaForm } from '@inertiajs/react';
import { Checkbox } from '@/components/ui/checkbox';
import {
    Field,
    FieldDescription,
    FieldError,
    FieldGroup,
    FieldLabel,
} from '@/components/ui/field';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';

export type CalendarEventFormData = {
    title: string;
    location: string;
    live_broadcast_url: string;
    starts_at: string;
    ends_at: string;
    description: string;
    remind_three_days_before: boolean;
    remind_one_day_before: boolean;
    remind_fifteen_minutes_before: boolean;
};

export function CalendarEventFormFields({
    form,
    timezone,
}: {
    form: InertiaForm<CalendarEventFormData>;
    timezone: string;
}) {
    return (
        <FieldGroup>
            <Field data-invalid={Boolean(form.errors.title)}>
                <FieldLabel htmlFor="event_title">Event title</FieldLabel>
                <Input
                    id="event_title"
                    value={form.data.title}
                    onChange={(event) =>
                        form.setData('title', event.target.value)
                    }
                    aria-invalid={Boolean(form.errors.title)}
                    placeholder="Lead Lab planning session"
                />
                <FieldError>{form.errors.title}</FieldError>
            </Field>

            <div className="grid gap-6 md:grid-cols-2">
                <Field data-invalid={Boolean(form.errors.location)}>
                    <FieldLabel htmlFor="event_location">Location</FieldLabel>
                    <Input
                        id="event_location"
                        value={form.data.location}
                        onChange={(event) =>
                            form.setData('location', event.target.value)
                        }
                        aria-invalid={Boolean(form.errors.location)}
                        placeholder="Lead Lab studio or online"
                    />
                    <FieldError>{form.errors.location}</FieldError>
                </Field>

                <Field data-invalid={Boolean(form.errors.live_broadcast_url)}>
                    <FieldLabel htmlFor="event_live_broadcast_url">
                        Live broadcast link
                    </FieldLabel>
                    <Input
                        id="event_live_broadcast_url"
                        type="url"
                        value={form.data.live_broadcast_url}
                        onChange={(event) =>
                            form.setData(
                                'live_broadcast_url',
                                event.target.value,
                            )
                        }
                        aria-invalid={Boolean(form.errors.live_broadcast_url)}
                        placeholder="https://example.com/live"
                    />
                    <FieldError>{form.errors.live_broadcast_url}</FieldError>
                </Field>
            </div>

            <FieldDescription>
                Location and the live broadcast link are optional.
            </FieldDescription>

            <div className="grid gap-6 md:grid-cols-2">
                <Field data-invalid={Boolean(form.errors.starts_at)}>
                    <FieldLabel htmlFor="event_starts_at">Starts</FieldLabel>
                    <Input
                        id="event_starts_at"
                        type="datetime-local"
                        value={form.data.starts_at}
                        onChange={(event) =>
                            form.setData('starts_at', event.target.value)
                        }
                        aria-invalid={Boolean(form.errors.starts_at)}
                    />
                    <FieldError>{form.errors.starts_at}</FieldError>
                </Field>

                <Field data-invalid={Boolean(form.errors.ends_at)}>
                    <FieldLabel htmlFor="event_ends_at">Ends</FieldLabel>
                    <Input
                        id="event_ends_at"
                        type="datetime-local"
                        value={form.data.ends_at}
                        onChange={(event) =>
                            form.setData('ends_at', event.target.value)
                        }
                        aria-invalid={Boolean(form.errors.ends_at)}
                    />
                    <FieldError>{form.errors.ends_at}</FieldError>
                </Field>
            </div>

            <FieldDescription>
                Event times are saved and displayed in {timezone}.
            </FieldDescription>

            <div className="space-y-3 rounded-lg border p-4">
                <div className="space-y-1">
                    <FieldLabel>Email reminders</FieldLabel>
                    <FieldDescription>
                        Send reminders to active, approved participants and
                        administrators with verified email addresses.
                    </FieldDescription>
                </div>
                <div className="grid gap-3 sm:grid-cols-3">
                    <label className="flex items-center gap-2 text-sm">
                        <Checkbox
                            checked={form.data.remind_three_days_before}
                            onCheckedChange={(checked) =>
                                form.setData(
                                    'remind_three_days_before',
                                    checked === true,
                                )
                            }
                        />
                        <span>3 days before</span>
                    </label>
                    <label className="flex items-center gap-2 text-sm">
                        <Checkbox
                            checked={form.data.remind_one_day_before}
                            onCheckedChange={(checked) =>
                                form.setData(
                                    'remind_one_day_before',
                                    checked === true,
                                )
                            }
                        />
                        <span>1 day before</span>
                    </label>
                    <label className="flex items-center gap-2 text-sm">
                        <Checkbox
                            checked={form.data.remind_fifteen_minutes_before}
                            onCheckedChange={(checked) =>
                                form.setData(
                                    'remind_fifteen_minutes_before',
                                    checked === true,
                                )
                            }
                        />
                        <span>15 minutes before</span>
                    </label>
                </div>
            </div>

            <Field data-invalid={Boolean(form.errors.description)}>
                <FieldLabel htmlFor="event_description">Description</FieldLabel>
                <Textarea
                    id="event_description"
                    value={form.data.description}
                    onChange={(event) =>
                        form.setData('description', event.target.value)
                    }
                    aria-invalid={Boolean(form.errors.description)}
                    placeholder="What should participants know before joining?"
                    rows={4}
                />
                <FieldError>{form.errors.description}</FieldError>
            </Field>
        </FieldGroup>
    );
}
